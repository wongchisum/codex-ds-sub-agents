...
预览正确后去掉 `--dry-run` 再执行真实清理。不要手工删除整个 `%USERPROFILE%\.codex`。
...
Windows 文档使用 `py -3`。先列出 Python Launcher 已识别的解释器：

```powershell
py -0p
```

如果没有 Python Launcher，使用 `Get-Command python -All` 检查候选路径，再显式运行非 Store 的 `python.exe`。安装器生成的 agent、凭证 helper 和 Task Scheduler 命令必须使用运行安装器的 `sys.executable`，不能写死 `python3` 或 WindowsApps shim。
