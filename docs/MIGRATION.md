...
The project and plugin package remain named `codex-custom-subagents`, while the active Skill is now `codex-custom-subagent`. Older releases used `$deepseek-delegation`, `$codex-custom-agents`, and `$codex-custom-subagents`. New installs use `~/.codex/skills/codex-custom-subagent/`.
...
1. Looks for `~/.codex/skills/deepseek-delegation/`, `~/.codex/skills/codex-custom-agents/`, and `~/.codex/skills/codex-custom-subagents/`.
...
5. Installs `codex-custom-subagent` and re-renders registered agent paths.

Migration is idempotent. An unmanaged old directory is reported and preserved for manual review.

## Compatibility boundary

New prompts and tasks must use `$codex-custom-subagent`; no active alias is installed for the legacy names, including `$codex-custom-subagents`.
